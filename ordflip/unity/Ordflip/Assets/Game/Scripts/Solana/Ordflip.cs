...
using Ordflip;
using Ordflip.Program;
using Ordflip.Errors;
using Ordflip.Accounts;
...
namespace Ordflip
{
    ...
    namespace Errors
    {
        public enum OrdflipErrorKind : uint
        {
    ...
    public partial class OrdflipClient : TransactionalBaseClient<OrdflipErrorKind>
    {
        public OrdflipClient(IRpcClient rpcClient, IStreamingRpcClient streamingRpcClient, PublicKey programId) : base(rpcClient, streamingRpcClient, programId)
        {
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.OrdflipProgram.InitPlayer(accounts, levelSeed, programId);
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.OrdflipProgram.ChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.OrdflipProgram.SuperChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }

        protected override Dictionary<uint, ProgramError<OrdflipErrorKind>> BuildErrorsDictionary()
        {
            return new Dictionary<uint, ProgramError<OrdflipErrorKind>>{{6000U, new ProgramError<OrdflipErrorKind>(OrdflipErrorKind.NotEnoughEnergy, "Not enough energy")}, {6001U, new ProgramError<OrdflipErrorKind>(OrdflipErrorKind.WrongAuthority, "Wrong Authority")}, };
        }
    }
    ...
    namespace Program
    {
        ...
        public static class OrdflipProgram
        {
